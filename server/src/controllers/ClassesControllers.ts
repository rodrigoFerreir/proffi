...
import convertHourToMinutes from "../utils/covertHoursToMinuts";
...
export default class ClassesController {
    ...
    async create(request: Request, response: Response) {
        ...
        try {
            const IdUsersInsert = await trx('users').insert({
                name, avatar, whatsapp, bio
            });

            const user_id = IdUsersInsert[0]; // a função inset retorna o id do ultimo dado inserido 
